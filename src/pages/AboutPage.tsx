...
import { Code, Brain, Atom, Paintbrush, Monitor, ShieldCheck, ShieldUser } from 'lucide-react';
...
const textClass = 'text-sm text-black dark:text-gray-200';
const highlightClass = 'font-semibold text-black dark:text-white';

const AboutPage = ({ isInViewport }: { isInViewport: boolean }) => {
  return (
    <div className="page-wrapper bg-gray-100 dark:bg-slate-800 flex flex-col items-center justify-center">
      <div className="relative">
        ...
        >
          <div className="flex flex-col-reverse sm:flex-row gap-8 items-start justify-start w-full">
            <div className="flex-1 h-full w-full flex flex-col grow items-start justify-start bg-gray-100 dark:bg-slate-800 flex-wrap max-h-100  shadow-lg rounded-xl p-2.5 gap-y-2.5">
              {ABOUT_ITEMS.map(item => (
                <div
                  key={item.title}
                  className="flex gap-x-3 flex-row bg-white dark:bg-gray-700 items-center justify-start w-full h-11  p-3 rounded-md shadow-md"
                >
                  <item.Icon
                    height={18}
                    width={18}
                  ...
                  />
                  <p className="text-sm dark:text-white text-black">{item.title}</p>
                </div>
              ))}
            </div>
            <div className="p-2 bg-gray-200 dark:bg-gray-700 rounded-xl shadow-xl shrink-0">
              <LazyLoadImage
                src={MY_PICTURE}
          ...
          <div className="p-2.5 max-w-164 w-full bg-gray-100 dark:bg-slate-800 shadow-lg rounded-xl mt-8">
            <div className="p-4 pl-3 bg-white dark:bg-gray-700 rounded-xl shadow-md">
